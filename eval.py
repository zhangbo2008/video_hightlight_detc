...
def compute_hl_hit1(qid2preds, qid2gt_scores_binary):
    ...
    for idx, qid in enumerate(qids):
        pred_clip_idx = qid2max_scored_clip_idx[qid] #预测的最好秒数.
        gt_scores_binary = qid2gt_scores_binary[qid]   # (#clips, 3) #答案.
        if pred_clip_idx < len(gt_scores_binary):
            hit_scores[idx] = gt_scores_binary[pred_clip_idx] #积累击中的分数.
...
def compute_hl_ap(qid2preds, qid2gt_scores_binary, num_workers=8, chunksize=50):
    ...
    for idx, qid in enumerate(qids):
        for w_idx in range(3):  # annotation score idx
            y_true = qid2gt_scores_binary[qid][:, w_idx] # 专家在当前帧的平分.
...
def eval_main():
    ...
    args.submission_path="standalone_eval/sample_val_preds.jsonl"
    args.gt_path="data/highlight_val_release.jsonl"
    args.save_path="standalone_eval/sample_val_preds_metrics.json"
